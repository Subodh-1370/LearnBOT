...
class EnhancedStateGame:
    """
    Enhanced Snake Game with improved state representation for better learning
    """
    
    def __init__(self, width=800, height=600, block_size=20, speed=40, 
                 headless=False, enable_animations=True):
        """
        Initialize Enhanced Snake Game with improved state representation
    ...
    def _init_display(self):
        ...
        pygame.display.set_caption('Enhanced Snake Game - LearnBot')
    ...
    def get_enhanced_state(self):
        """
        Get enhanced state representation with additional features
        
        Returns:
            numpy array: Enhanced state vector with 20+ features
        """
        head = self.snake[0]
        ...
        # Enhanced features
        # Distance to food (1 feature)
        food_distance = abs(head[0] - self.food[0]) + abs(head[1] - self.food[1])
        food_distance_normalized = food_distance / (self.w + self.h)
        
        # Snake length (1 feature)
        snake_length_normalized = len(self.snake) / (self.w * self.h)
        
        # Time since last food (1 feature)
        time_since_food_normalized = min(self.steps_since_last_food / 100, 1.0)
        
        # Wall proximity (4 features)
        wall_left = head[0] / self.w
        wall_right = (self.w - 1 - head[0]) / self.w
        wall_up = head[1] / self.h
        wall_down = (self.h - 1 - head[1]) / self.h
        
        # Body proximity (1 feature)
        body_proximity = 0
        for segment in self.snake[1:]:
            distance = abs(head[0] - segment[0]) + abs(head[1] - segment[1])
            if distance < 5:
                body_proximity += 1
        body_proximity_normalized = min(body_proximity / len(self.snake), 1.0)
        
        # Total steps (1 feature)
        total_steps_normalized = min(self.total_steps / 1000, 1.0)
        ...
        state = np.array([
            ...
            time_since_food_normalized,
            wall_left, wall_right, wall_up, wall_down,
            body_proximity_normalized,
            total_steps_normalized
        ])
        
        return state

# Backward compatibility alias
EnhancedSnakeGame = EnhancedStateGame

if __name__ == '__main__':
    game = EnhancedStateGame(speed=10)
