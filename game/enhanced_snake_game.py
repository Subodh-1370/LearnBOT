...
# Add parent directory to path for asset manager
...
class EnhancedSnakeGame:
    """
    Enhanced Snake Game with professional visuals, animations, and headless mode
    Maintains full compatibility with RL training pipeline
    """
    ...
    def __init__(self, width=800, height=600, block_size=20, speed=40, 
        ...
        self.grid_color = (30, 35, 50)  # Subtle grid lines
        self.ui_bg_color = (15, 20, 35)  # UI background
    ...
    def _init_fonts(self):
        """Initialize fonts"""
        self.score_font = asset_manager.load_font("score", 28)
        self.ui_font = asset_manager.load_font("ui", 20)
        self.game_over_font = asset_manager.load_font("game_over", 48)
    ...
    def play_step(self, action):
        ...
        # 1. Collect user input (only in non-headless mode)
        if not self.headless:
    ...
    def _update_ui(self):
        """Update the game display with enhanced visuals"""
        if self.headless:
            return
    ...
    def _draw_ui_panel(self):
        """Draw the UI panel with score and info"""
    ...
    def get_state(self):
        """
        Get current state representation (unchanged for RL compatibility)
...
if __name__ == '__main__':
    ...
    game = EnhancedSnakeGame(speed=10)
